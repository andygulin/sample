...
public class JdbcTemplateTest {
    ...
    public void insert() {
        ...
        int result = this.jdbcTemplate.update(sql, ps -> {
            ps.setString(1, "bbb");
            ps.setInt(2, 12);
    ...
    public void insert3() {
        ...
        int result = this.jdbcTemplate.update(con -> {
    ...
    public void queryObj() {
        User user = this.jdbcTemplate.query("SELECT * FROM `user` WHERE id=?", rs -> {
            rs.next();
            User u = new User();
    ...
    @Test
    public void execute() {
        int result = this.jdbcTemplate.execute(con -> {
            CallableStatement statement = con.prepareCall("UPDATE `user` SET name=? WHERE id=?");
            statement.setString(1, "zzz");
            statement.setInt(2, 1);
            return statement;
        }, (CallableStatementCallback<Integer>) statement -> statement.executeUpdate());
        logger.info(result);
    }
    ...
    public void execute2() {
        int result = this.jdbcTemplate.execute(con -> {
            ...
            statement.setInt(2, 1);
            return statement;
        }, (PreparedStatementCallback<Integer>) ps -> ps.executeUpdate());
        logger.info(result);
    }
